...
function openPermModal(idx){
    ...
    PERMS.forEach(function(p,i){
        ...
        html+='<div class="col-md-6 mb-2"><div class="d-flex align-items-center border rounded p-2"><span class="flex-grow-1 small fw-bold">'+p.label+'</span><label class="mb-0 me-3 small text-success"><input type="radio" name="perm_'+p.bit+'" value="grant" '+(g||')+> G</label><label class="mb-0 me-3 small text-danger"><input type="radio" name="perm_'+p.bit+'" value="deny" '+(d||')+> D</label><label class="mb-0 small text-muted"><input type="radio" name="perm_'+p.bit+'" value="none" '+(!g&&!d?'checked':'')+'>–</label></div></div>';
    });
